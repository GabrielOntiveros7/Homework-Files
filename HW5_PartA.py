...
def ff(Re, rr, CBEQN=False):
    ...
    if CBEQN:
        ...
        result = fsolve(cb, 0.02, maxfev=5000)
        return result[0]
    else:
        return 64 / Re
...
def plotMoody(plotPoint=False, pt=(0, 0)):
    ...
    ReValsCB = np.logspace(np.log10(4000.0), 8, 200)  # for use with Colebrook equation
    ReValsL = np.logspace(np.log10(600.0), np.log10(2000.0), 20)  # for use with Laminar flow
    ReValsTrans = np.logspace(np.log10(2000.0), np.log10(4000.0), 20)  # for use with Transition flow
    ...
    # Step 3:  calculate the friction factor in the laminar and transition ranges
    ...
    plt.loglog(ReValsL, ffLam, 'k-')
    plt.loglog(ReValsTrans, ffTrans, 'k--')
    for nRelR in range(len(ffCB)):
        plt.loglog(ReValsCB, ffCB[nRelR], color='k', label=nRelR)
        plt.annotate(text=f'{rrVals[nRelR]:g}', xy=(ReValsCB[-1], ffCB[nRelR][-1]))
